...
- **Not found is exit 4** (`Status.Code 90`) — re-check the code. But wrong
  credentials also exit 4, with a bare `HTTP 404` message (see Tooling) — read
  the message before re-resolving anything.
