...
test("an empty 200 body or a 204 is a DestatisParseError, not null", async () => {
  for (const res of [rawResponse("", "application/json"), rawResponse("  \n", "application/json"), rawResponse("", "application/json", 204)]) {
    ...
    await assert.rejects(
      ...
      (err) => err instanceof DestatisParseError && err.message === "Empty response body from /find/find",
    );
    await assert.rejects(() => e.getJson("/helloworld/whoami"), DestatisParseError);
  }
});
...
test("a 429 with Retry-After in seconds waits that long before each retry", async () => {
  ...
  await assert.rejects(
    () => engine.postJson("/x", {}, {}),
    (e: unknown) => e instanceof DestatisApiError && e.httpStatus === 429,
  );
