...
test("performs a real GET and returns status, headers and body", async () => {
  await withServer(
    ...
    async (baseUrl) => {
      const resp = await nodeHttpTransport({ method: "GET", url: `${baseUrl}/genesisWS/rest/2020/helloworld/whoami` });
...
test("rejects an unsupported protocol with DestatisNetworkError", async () => {
  ...
  await withServer(
    ...
    async (baseUrl) => {
      ...
      await assert.rejects(
        ...
        (err) => err instanceof DestatisNetworkError && /timed out after 300ms/.test(err.message),
      );
