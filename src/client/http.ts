...
import { DestatisNetworkError } from "./errors.js";
...
export const nodeHttpTransport: Transport = (request) =>
  new Promise<HttpResponse>((resolve, reject) => {
    ...
    try {
      ...
      reject(new DestatisNetworkError(`Invalid URL: ${request.url}`));
      return;
    }
    ...
    if (url.protocol !== "http:" && url.protocol !== "https:") {
      reject(new DestatisNetworkError(`Unsupported protocol "${url.protocol}" in URL: ${request.url}`));
      return;
    }
      ...
      (res) => {
        ...
        res.on("data", (chunk: Buffer) => {
          ...
          if (maxBytes !== undefined && received > maxBytes) {
            aborted = true;
            res.destroy();
            reject(new DestatisNetworkError(`Response exceeded maxResponseBytes (${maxBytes})`));
            return;
          }
          chunks.push(chunk);
        });
        ...
        res.on("error", (err) => {
          ...
          reject(new DestatisNetworkError(`Response stream error: ${err.message}`, { cause: err }));
        });
      },
    );
    ...
    if (request.timeoutMs && request.timeoutMs > 0) {
      req.setTimeout(request.timeoutMs, () => {
        req.destroy(new DestatisNetworkError(`Request timed out after ${request.timeoutMs}ms`));
      });
    }

    req.on("error", (err) => {
      // A timeout destroy already passes a DestatisNetworkError; don't double-wrap.
      reject(
        err instanceof DestatisNetworkError ? err : new DestatisNetworkError(err.message, { cause: err }),
      );
    });
